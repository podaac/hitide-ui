//>>built
define(["dojo/_base/kernel", "dojo/_base/declare", "dojo/_base/lang", "dojo/_base/array", "dojo/sniff", "dojo/string", "dojox/xml/parser", "../kernel", "../lang", "../request", "../WKIDUnitConversion", "../SpatialReference", "../geometry/Point", "../geometry/Extent", "../geometry/webMercatorUtils", "./TiledMapServiceLayer", "./TileInfo", "./WMTSLayerInfo", "dojo/query"], function(g, D, w, k, z, K, E, L, x, F, y, A, B, C, G, H, I, J) {
    return D([H], {
        ...
        constructor: function(a, b) {
            ...
            b.layerInfo && (this.layerInfo = b.layerInfo, this._identifier = b.layerInfo.identifier,
                this._tileMatrixSetId = b.layerInfo.tileMatrixSet, b.layerInfo.format && (this.format = "image/" + b.layerInfo.format), this._style = b.layerInfo.style, this.title = b.layerInfo.title, this._dimension = b.layerInfo.dimension);
        ...
        getTileUrlTemplate: function(a) {
            ...
            if (a) {
                ...
                } else c = a.formats[0], -1 === c.indexOf("image/") && (c = "image/" + c); if (e) {
                    if (-1 === k.indexOf(a.styles, e)) {
                        console.error("The layer doesn't support the style of " + e);
                        this.onError(Error("The layer doesn't support the style of " +
                            e));
                        return
                    }
                } else e = a.styles[0]; if (!d && a.dimensions) d = a.dimensions[0];
                ...
                if (f) {
                    if (h = k.filter(a.tileMatrixSetInfos, function(a) {
                        ...
                        this.onError(Error("The tileMatrixSetId " + f + " is not supported by the layer of " +
                            b));
                        return
                    }
                } else(h = k.filter(a.tileMatrixSetInfos, function(a) {
                    return "GoogleMapsCompatible" === a.tileMatrixSet
                })[0]) || (h = a.tileMatrixSetInfos[0]), f = h.tileMatrixSet;
        ...
        _getTileUrlTemplate: function(a, b, f, c, e) {
            ...
            f || (f = this.format);
            c || (c = this._style);
            if (this.resourceUrls && 0 < this.resourceUrls.length) return d = this.resourceUrls[0].template,
                d.indexOf(".xxx") === d.length - 4 && (d = d.slice(0, d.length - 4)), d = d.replace(/\{Style\}/gi, c), d = d.replace(/\{TileMatrixSet\}/gi, b), d = d.replace(/\{TileMatrix\}/gi, "{level}"), d = d.replace(/\{TileRow\}/gi, "{row}"), d = d.replace(/\{TileCol\}/gi, "{col}"), d = d.replace(/\{dimensionValue\}/gi, e);
        ...
        _parseCapabilities: function(a) {
            ...
            if (b) {
                ...
                k.forEach(a, function(a) {
                    ...
                    s.push(h);
                    this.layers.push(this._getWMTSLayerInfo(h, a, b))
                }, this);
                this._setActiveLayer();
                this.loaded = !0;
                this.onLoad(this)
        ...
        _setActiveLayer: function(a) {
            ...
            if (this.layers)
                if (this._identifier ? a = k.filter(this.layers, function(a) {
                    ...
                    } else this.format = a.formats[0], -1 === this.format.indexOf("image/") && (this.format = "image/" + this.format); if (this._style) {
        ...
        _getWMTSLayerInfo: function(a, b, f) {
            var c = this._getTagValues("Abstract", b)[0],
                ...
                d = g.query("WGS84BoundingBox", b)[0],
                h = d ? this._getTagValues("LowerCorner", d)[0].split(" ") : ["-180", "-90"],
                ...
                t = parseFloat(s[0]),
                s = parseFloat(s[1]),
                d = new C(d, h, t, s, new A({
                    wkid: 4326
                })),
                s = this._getTagValues("Identifier", g.query("Style", b)[0]),
                u = this._getTagValues("Identifier", g.query("Dimension", b)[0]),
                m = this._getTagValues("Value", g.query("Dimension", b)[0]) || this._getTagValues("Default", g.query("Dimension", b)[0]),
                h = this._getTagValues("Format", b);
                ...
                if (0 < this._allMatrixInfos.length)
                    for (f = 0; f < this._allMatrixInfos.length; f++)
                        if (this._allMatrixInfos[f].tileMatrixSet == d) {
                            g = this._allMatrixInfos[f];
                            break
                        }
                g || (g = this._getLayerMatrixInfo(d, a, b), this._allMatrixInfos.push(g));
                c.push(g)
            }, this), c
        },
        _getLayerMatrixInfo: function(a, b, f) {
            ...
            var t = this._getTagValues("TileMatrix", b),
                ...
                m = this._getTagValues("SupportedCRS",
                    u)[0];
            c = parseInt(m.split(":").pop());
            if (900913 == c || 3857 == c) c = 102100;
            if (-1 < m.toLowerCase().indexOf("crs84") || -1 < m.toLowerCase().indexOf("crs:84")) c = 4326, h = !0;
            else if (-1 < m.toLowerCase().indexOf("crs83") || -1 < m.toLowerCase().indexOf("crs:83")) c = 4269, h = !0;
            else if (-1 < m.toLowerCase().indexOf("crs27") || -1 < m.toLowerCase().indexOf("crs:27")) c = 4267, h = !0;
            var r = new A({
                    wkid: c
                }),
                l = g.query("TileMatrix", u)[0];
            ...
            var q = parseFloat(q),
                v = parseFloat(v),
                w = h && 4326 === c && 90 === q && -180 === v;
            for (e = 0; e < this._flippingAxisForWkids.length; e++)
                if (m.split(":").pop() >= this._flippingAxisForWkids[e][0] && m.split(":").pop() <= this._flippingAxisForWkids[e][1] || 4326 === c && (!h || w)) {
                    4326 === c && 90 < q && (q = "90");
                    d = new B(v, q, r);
                    break
                }
            e === this._flippingAxisForWkids.length &&
                (d = new B(q, v, r));
            if (0 === t.length) {
                t = g.query("TileMatrix", u);
                for (e = 0; e < t.length; e++) h = this._getLodFromTileMatrix(t[e], c, e), k.push(h)
            } else
                for (e = 0; e < t.length; e++) h = this._getTagWithChildTagValue("TileMatrix", "Identifier", t[e], u), h = this._getLodFromTileMatrix(h, c, e), k.push(h);
            c = g.query("BoundingBox", u)[0];
            var n, p;
        ...
        _getTagValues: function(a, b) {
            ...
            e = g.query(c[0], b)[0];
            if (1 < c.length) {
                for (d = 1; d < c.length - 1; d++) e = g.query(c[d], e)[0];
                c = g.query(c[c.length - 1], e)
            } else c = g.query(c[0],
                b);
            c && 0 < c.length && k.forEach(c, function(a) {
                z("ie") ? f.push(a.childNodes[0].nodeValue) : f.push(a.textContent)
            });
            return f
        },
        ...
        _getTagWithChildTagValue: function(a, b, f, c) {
            ...
            for (d = 0; d < c.length; d++)
                if (-1 < c[d].nodeName.indexOf(a) && (z("ie") ? x.isDefined(g.query(b, c[d])[0]) && (e = g.query(b, c[d])[0].childNodes[0].nodeValue) : x.isDefined(g.query(b, c[d])[0]) && (e =
                    g.query(b, c[d])[0].textContent), e === f || f.split(":") && e === f.split(":")[1])) return c[d]
        },
