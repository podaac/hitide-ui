...
terraform import -var-file=tfvars/"${tf_venue}".tfvars -var="credentials=~/.aws/credentials" -var="profile=ngap-service-${tf_venue}" -var="app_version=${app_version}" aws_s3_bucket.hitide-site-bucket podaac-services-${tf_venue}-hitide || true
